...
public class ExerciseService {
    ...
    public void addExercise(Exercise ex) {
        String sql = "INSERT INTO exercise (plan_id, title, description, duration, difficulty_level, image) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public List<Exercise> getExercisesByPlan(int planId) {
        ...
        String sql = "SELECT exercise_id, plan_id, title, description, duration, difficulty_level, image " +
                "FROM exercise WHERE plan_id=? ORDER BY exercise_id DESC";
        ...
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    e.setPlanId(rs.getInt("plan_id"));
                    e.setTitle(rs.getString("title"));
                    e.setDescription(rs.getString("description"));
                    e.setDuration(rs.getInt("duration"));
                    e.setDifficultyLevel(rs.getString("difficulty_level"));
    ...
    public void updateExercise(Exercise ex) {
        String sql = "UPDATE exercise SET title=?, description=?, duration=?, difficulty_level=?, image=? " +
                "WHERE exercise_id=?";
